...
class Zend_Acl
{
    ...
    public function get($resource)
    {
        ...
        if (!$this->has($resource)) {
			...
			// original Acl.php file is saved as 'Acl_backup12072013.php'
			
			//the next line commented out inorder to correct SEO-friendly url errors, when the user tries to modify the url in detail page
			
            //throw new Zend_Acl_Exception("Resource '$resourceId' not found");
			
			//the next line added by me Chuks Ndubueze 12072013 to redirect to r=error page when user attempted to modify SEO-frienly url
			$request = Zend_Controller_Front::getInstance()->getRequest();
			$request->setControllerName('error');
			$request->setActionName('error');
        }
		
		//the next line commented out inorder to correct "Notice: Undefined index: details in C:\Apache2.2\htdocs\ride\library\Zend\Acl.php on line 372 (line 372 is next line below)"
